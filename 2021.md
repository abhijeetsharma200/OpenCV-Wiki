...
### *__Minutes__*
*  1. Release:
   * a. Opencv-python. Packages build is on track. All available configurations are migrated to Github Actions.
   * b. Xperience AI team tests packages now.
   * c. Girgory will send notification when packages are available.
*  2. 5.0 Tasks:
   * a. New text rendering affects videoio tests (frames with text are used). A lot of tests fails with low PSNR. Alexander Alekhin looks how to fix the issue with minimal test data changes
   * b. RGBD work from Rostislav is merged to contrib and should be forwarded to OpenCV next.
   * c. No progress with Python for now.
   * d. Python packages renaming:
      * i. Need to check import priorities: pure binary, archive, folder with pys
      * ii. Need to check Pypi rules about it.
   * e. Probably need to rename opencv2/*.hpp to opencv/*.hpp. At once, contrib’s headers should be renamed to opencv_contrib/*.hpp.
...
### *__Minutes__*
*  1. Release:
   * a. Release tag ETA 12:00 UTC. Release binaries ETA - EOW.
   * b. Python release will be done next week. ETA: next Friday.
   * c. Final reminder: changelog on Wiki
...
### *__Minutes__*
*  1. GSoC 2021:
   * a. Alexander Smorkalov will post blog on habr.com
   * b. Lydia will ask the remaining GSoC 2020 authors about blog post

*  2. PR & Issues:
   * a. Contrib - merge
   * b. Python bindings: Alexander Alekhin will file bug on Github. Alexander Smorkalov will involve Vadim Levin to resolve the issue.
   * c. Add reading of specific images from multipage tiff [#19780](https://github.com/opencv/opencv/pull/19780) - Commented in PR.
