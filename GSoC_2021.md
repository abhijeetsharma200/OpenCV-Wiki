...
1. #### _IDEA:_ Improve Python package with type annotations and type checking
   * ***Description:*** OpenCV Python interfaces are moving towards the type safety (e.g. strict type checking in argument parsing, overload resolution) and new functionality that is related only to Python might be on the way. Improve Python package of the OpenCV with type annotations and type checking.
