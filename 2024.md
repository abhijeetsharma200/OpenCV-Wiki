...
* Alexander S (opencv.ai):
  ...
  * Done:
    * Rostislav Vasilikhin:
      * Prepared HAL for setTo() on FastCV side (draft PR #25819) with instructions to reproduce on a device
      * In contradiction to customer's performance measurements, our measurements show that FastCV HAL is significantly slower regardless operation mode used
      * True reasons are still unclear, investigating and discussing measurement practices
      * HAL for moments() was requested to be changed, rejected
      * FastCV extensions for OpenCV:
        * Recursive bilateral filter: prepared, stored at our Gitlab
        * FAST10: in progress, will be finished soon
        * FFT/DCT: in progress, will be finished soon
        * fcvFilterThresholdRange and fcvMeanShift should not probably be covered by HAL
    * Alexander Smorkalov:
