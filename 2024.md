...
* Alexander S (opencv.ai):
  * Done:
    * GSoC 2024: trying to finalize the project on GTK4 + OpenGL support in OpenCV. Most likely a separate branch of code is needed to handle {GTK3 and GTK4} + OpenGL properly.
    * Some improvements in DNN have been done by Abdurraheem, such as HardMax etc.
    * Added support for the fresh versions of cudnn into OpenCV; there was problem with std::tuple.
    * Alexander L can take part in OpenCV part time for now. He is going to submit faster blobFromImage.
