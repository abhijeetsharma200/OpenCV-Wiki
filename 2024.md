...
* Yuantao F (OpenCV China):
  * Merged PR #26505 with nearest-neighbor warpaffine/warpperspective/remap acceleration. Started working on a faster C3 branch of those 3 functions (all data types (U8, U16, FP32), bilinear and nearest-neighbor interpolation).
  * Started performance testing of OpenCV on K1 (RISC-V + RVV 1.0) and RK3568 (ARMv8.2 + NEON w. FP16 support) chips.
* Gursimar:
  * object detection example refactoring PR has been merged.
  * tracker example has been refactored; PR not merged yet.
  * working on composing a list of algorithms for the revised photo module in OpenCV 5.0/5.x.
* Vincent:
  * still have blocker issue: https://github.com/opencv/opencv/issues/26497
  * there are some false positives on address sanitizer used in OpenCV on ARM.
  * still working on the proper implementation of InputArray/OutputArray that will wrap `std::vector<T>` for different `T`'s properly.
* Phil:
  * working on bin picking OpenCV challenge.
  * trying to make docker-based environment to evaluate OpenCV competition submissions more secure.
  * got some individual donations to OpenCV lately.
  * making final arrangements for OpenCV F2F next week.
* Gary:
  * looking at datasets for robot arm manipulations.
  * camera stabilization functionality (as well as other nice image/video processing algorithms for robotics) could be useful to have
* Alexander S (opencv.ai):
  * Done:
    * Abduragim Shtanchaev:
      * Working on GPT2 inference sample to make it dynamic
    * Alexander Smorkalov:
      * Prepared 5.0-alpha release: [checklist](https://docs.google.com/spreadsheets/d/1bsu3zukr10MCm1c_DPCmAkagiRK_5lboCNQuKofK5ps/edit?gid=1285899438#gid=1285899438).
      * Refreshed OpenCV-Python repository:
      * Updated dependencies on Linux
      * Partially solved actions/checkout@3 step issue on Github
      * Updated MacOS to version 13.
      * Refreshed setup.py and related scripts in 5.x branch.
      * Merged 4.x->5.x.
      * Works on Mac M1 update for CI.
      * Prepared 4.x->5.x merge #26546
      * Fixed constants and types issue in Java, Obj-C and Swift bindings ## 26570, 26568, 26559
      * Finished initial FastCV integration as HAL and opencv_contrib modules #26556,  26316, contrib ## 3824 3811, 3839.
      * Reviewed, tested and helped to resolve issues with incoming PRs.
    * Rostislav Vasilikhin:
      * boxFilter 5x5 perf tests: done [#26560](https://github.com/opencv/opencv/pull/26560) [#1221@extra](https://github.com/opencv/opencv_extra/pull/1221)
      * get optimal camera matrix: in progress
      * Added 8UC2 to cv::rotate() perf tests: [#26544](https://github.com/opencv/opencv/pull/26544)

## 2024-11-27

* Yuantao F (OpenCV China):
  * Yuantao F:
    * Accelerated nearest-neighbor interpolation mode for warpaffine, warpperspective, remap: #26505. Most of the issues have been resolved.
    * Updated 5.x containers for CI.
  * Jia W:
    * Annoy algorithm now produces deterministic results (multi-tree search Index), not matter how many cores are used/available. Now need to evaluate accuracy and query speed using the produced index.
    * reviewed several samples.
* Vadim:
  * helped to fix Tensorflow importer and the respective tests
* Vincent:
  * Whether OpenCV 5 will support custom arrays?
* Shiqi:
  * Finalizing OpenCV F2F program (titles and abstracts need to be sent by Nov 30th)
* Gary:
  * Preparing for OpenCV workshop in May, negotiating with possible speakers.
* Gursimar:
  * updated the PRs for object detection and tracker samples. Both the pull requests have been approved by Jia and should be ready to merge.  
* Alexander S (opencv.ai):
  * Done:
    * Abduragim Shtanchaev:
      * on vacation
    * Alexander Smorkalov:
      * Finished migration of G-API to OpenCV contrib: [#3827](https://github.com/opencv/opencv_contrib/pull/3827). Fixed issue with G-API python bindings #26527, updated CI.
      * Bumped OpenCV version in 5.x #26540.
      * Reviewed, tested, helped to resolve issues #26520,  26526, 26394, 26511, 26513, 26508, 26514, 26503, 26473, 26534, 26533, 26531, 26530, 26512, contrib ## 3831, 3830, 3829, CI ## 200, 198
    * Alexander Lyulkov:
      * Vacation
    * Rostislav Vasilikhin:
      * [#25597](https://github.com/opencv/opencv/pull/25597) reviewed, requested changes

## 2024-11-20

* Yuantao F (OpenCV China):
  * Yuantao F:
    * submitted PR with performance regressions in warpPerspective, warpAffine & remap (merged).
    * working on nearest-neighbor interpolation flavor of warping functions
  * Jia W:
    * reviewed samples by Gursimar. all the found issues have been merged.
    * parallel Annoy is made deterministic. Now need to evaluate quality.
* Vadim:
  * helped to fix LSTM
  * removed G-API module from the main repo; to be moved to opencv_contrib.
  * fixed Python detection on macOS (not merged yet)
  * invited A. G. to present 'OpenCV @ HF' initiative.
* Vincent:
  * C++ calibration ported from 5.x back to 4.x.
  * Observed degradation in calibration accuracy (C++ version of Levenberg-Marquardt solver in 4.x is worse than the old C implementation, but C++ implementation of LevMarq in 5.x should be better than both).
* Shiqi:
  * The new Wechat QR code: just finished evaluation. Will prepare PR within 1-2 weeks.
  * Now preparing program for OpenCV F2F; the participants should provide titles and abstracts by Nov 30th.
* Gursimar:
  * finalized tracker and fixed bugs in a few samples.
  * object detection. It uses some API that is not implemented in the new engine.
* Phil:
  * working on the new competition. Looking on how to employ container (docker) technology to make it safer for evaluation.
  * "special AI conference" in 2025 is another event that opencv.org team is working on.
* Alexander S (opencv.ai):
  * Done:
    * Abduragim Shtanchaev:
      * Finished LSTM for new graph 🎉 [#26391](https://github.com/opencv/opencv/pull/26391)
      * Working on running diffusion model on new graph engine to check if dynamic shape are supported
    * Alexander Smorkalov:
      * Prepared 4.x->5.x merge #.
      * Dropped C API in Core module #26458
      * Tested and benchmarked FastCV integration as HAL #26316
      * Updated CI for G-API drop.
      * Reviewed, tested, helped to resolve issues with #26478, 26469, 26468, 26466, 26462, 26461, 26460, 26459, 26455, 26454, 26485, 26484, 26316, 26391
    * Alexander Lyulkov:
      * Working on modifying Tensorflow parser for the new dnn engine.
* Vladimir D:
  * removed the remaining traces of C API from 5.x
  * updated ffmpeg wrappers for Windows.
...
* Vadim:
  ...
  * [OpenCV China] started working on a faster median filter: https://github.com/opencv/opencv/pull/26422. The goal is to replace the current "O(N)" and "O(1)" algorithms with the new "O(N)" algorithm which is significantly faster.
  * [OpenCV China] continue working on replacing FLANN with Annoy. We found non-deterministic behaviour in Annoy, which has been also reported in the upstream repo; now trying to fix it.
  * [OpenCV China] merged patch to 5.x that renames features2d to features, according to https://github.com/opencv/opencv/issues/24999
* Vincent:
  * TBD
* Phil:
  * OpenCV competition on bin picking (using robot arm); probably, could be a CVPR workshop, but also have ideas how to do it independently, as a regular OpenCV competition.
  * PyCharm + OpenCV was presented last week with great success.
* Shiqi:
  * wechat QR detector and decoder. Students compared the new version by wechat team and the current "traditional CV" implementation.
  * different implementations of aruco marker detection is in progress.
  * libcamera backend for OpenCV is also in progress.
* Alexander S (opencv.ai):
  * Done:
    * Abduragim Shtanchaev:
      * Finishing refactoring of LSTM. The goal is to finally repair OCR models.
      * Fixed LSTM bidirectional for any batchsize #26391.
      * Some issue with test accuracy on TextDetection test. Working on it.
    * Alexander Smorkalov:
      * Prepated 4.x->5.x merge #26404, #26453
      * Added Universal Windows Package build to CI #26402, workflow #188.
      * Reviewed tested, helped to resolve issues ## 26364, 26396, 26418, 26394, 26405, 26414, 26403, 26379, 26401, 26400, 26397, 26390, 26254, 26383, 26384, 26391, 25624, 26446, 26444, 26443, 26420, 26434, 26405, 26155, 26437, 26379, 26435, 26414, 26348, 26451, contrib ## 3821, 3803, 3815, 3818, 3820, 3803.
      * Reworked KleidiCV integration #26303
      * Reworked FastCV integration to main #26316 and contrib #3811 repos. Pushed review changes to the own OpenCV fork branch FastcvHAL_1stPost.
...
* Phil:
  ...
  * got links to some quite interesting OpenCV-based projects:
    ...
    * https://github.com/mikolajlubiak/terminal_animation Turn media (video/gif/image) into colored, animated ASCII art.
* Vincent:
...
* Shiqi:
  ...
  * Contacted to Aruco author; will have meeting with him next week.
* Alexander S (opencv.ai):
  * Done:
    * Abduragim Shtanchaev:
...
* Alexander S (opencv.ai):
  * Done:
    ...
    * Rostislav Vasilikhin:
      * More types added to mixed arithmetics tests: [#26134](https://github.com/opencv/opencv/pull/26134)
