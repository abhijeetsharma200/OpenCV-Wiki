...
* Vadim:
  * resumed the work on the new version of DNN. Fixed compile errors and now working on repairing crashing tests
  * [OpenCV China] CANN-based backend for DNN has been finally merged
  * [OpenCV China] the patch that removes the old convolution and replaces it all with the new convolution has been improved. There is still performance regression for some 1D convolution cases (on ARM v8), but otherwise the new convolution is faster or much faster than the old one. Besides, a large chunk of code is now removed from DNN.
* Vincent:
  * submitted patches for Python tests and samples to make them compatible with fresh versions of numpy: https://github.com/opencv/opencv/pull/22965, https://github.com/opencv/opencv_contrib/pull/3399/files that should probably be integrated into OpenCV 4.7.0
  
* Alexander S (x.ai):
  * Done:
    * Rostislav:
      * [Issue #19803](https://github.com/opencv/opencv/issues/19803): found out that there’s probably no mistake in sigma2 calculation, proposed 2 PRs to fix it ([PR #22992](https://github.com/opencv/opencv/pull/22992), backport to 4.x [PR #22996](https://github.com/opencv/opencv/pull/22996)), both wrong
    * Alexander Panov:
      * Finished and merged Aruco migration to main repo
      * Finished and merged alignment markers detection for QR codes.
    * Alexander Smorkalov:
      * Reviewed, tested, helped to resolve issues with #22965, 22914, 22986, 22905, 21407, 22967, 22963, 22994, 22981, 20367, contrib #3371, 3396,
      * Fixed Java Script bindings after aruco migration #23009
      * Reviewed open issues related to arm CPUs and tickets for imgproc and objdetect (qrcode) modules. Reproduced several issues,   added technical details.
      * Finished environment update for OpenCV-Python build. Works on release preparation.
      * Sergei Shutov:
      * Fixed PR #22962 and compared performance tests before and after patch applying.
      * Prepared PR #22967 Fix maxIter parameter in usac findEssentialMat
      * Prepared PR #3396 in opencv_contrib - fix aruco markers and board detection tutorials
  * In progress:
    * Rostislav:
      * [Issue #19803](https://github.com/opencv/opencv/issues/19803): discussing the issue with other participants
    * Alexander Panov:
      * Charuco migration to main repo and calibration tools update.
    * Alexander Smorkalov:
      * OpenCV 4.7.0 release preparation.
    * Sergei Shutov:
      * Fixing aruco tutorials, preparing PR to main repo
...
* Shiqi:
  * the new better tracking models will hopefully be contributed to OpenCV soon.
  * the new pose estimation model might be contributed to OpenCV as well.
* Alexander S (x.ai):
  * Done:
    * Rostislav:
...
* Alexander S (x.ai): 
  * Done:
    ...
    * Alexander Smorkalov:
      ...
      * Debugged OpenCV cv::VideoCapture failure with new camera stack on Raspberry Pi. The issue is on the system side and reproducible with other tools like ffmpeg #33820.
