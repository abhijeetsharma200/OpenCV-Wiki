...
* Vadim:
  * almost finished 1D/0D patch. The patch to OpenCV has passed CI already, contrib is still in progress; found that cuda* modules are super-slow to compile. Suggest to replace many of the manually written cuda kernels with calls to NVidia NPP: 1) OpenCV already calls some NPP functions 2) it should be more relevant and easier-to-support solution 3) compile speed should increase significantly, the size of binaries should decrease.
